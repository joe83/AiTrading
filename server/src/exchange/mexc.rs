    ...
    async fn connect(&mut self) -> Result<()> {
        if self.config.api_key.trim().is_empty() || self.config.secret_key.trim().is_empty() {
            *self.connected.write().await = false;
            anyhow::bail!("MEXC credentials not configured (API key or secret missing)");
        }

        // Verify credentials with authenticated account check
        let mut params = HashMap::new();
        self.signed_get("account", &mut params).await?;
        info!("MEXC account authenticated and connected.");
